...
class Program
{
    static void Main(string[] args)
    {
        ...
        //  debtors.Where(p=>DateTime.Now.Year-p.Birthday.Year>26& DateTime.Now.Year - p.Birthday.Year<36).ToList().ForEach(p=> Console.WriteLine(p));
        //debtors.Where(p => p.Debt <= 5000).ToList().ForEach(p => Console.WriteLine(p));
        //  debtors.Where(p=> p.FullName.Length>18& 
        // debtors.Where(p=>p.Birthday.Month==12|| p.Birthday.Month == 1|| p.Birthday.Month == 2).ToList().ForEach(p=> Console.WriteLine(p));

        /*       int allDebt=0;
               foreach(Debtor i in debtors)
               {
                   allDebt+=i.Debt;
               }
               double averageDebt=allDebt/debtors.Count;
               Console.WriteLine($"Average debt ={averageDebt}");
               debtors.Where(p=>p.Debt>averageDebt).ToList().ForEach(p => Console.WriteLine(p));*/
        /*  List<Debtor> SortedList = debtors.OrderBy(o => o.Debt).ToList();
          debtors.Reverse();
          foreach (var debtor in debtors)
          {
              Console.WriteLine(debtor);
          }
          List<Debtor> SortedList2 = debtors.OrderBy(o => o.FullName).ToList();
          foreach (var debtor in SortedList2)
          {
              Console.WriteLine(debtor);
          }*/
        //debtors.Where(p => !p.Phone.Contains("8")).ToList().ForEach(p => Console.WriteLine(p));
      /*  debtors = debtors.OrderBy(o => o.Debt).ToList();
        debtors.Reverse();
        for (int i = 0;i<5;i++)
        {
    ...
    //  debtors.Where(p=> p.Birthday.Year>1939 & p.Birthday.Year<1945).ToList().ForEach(p=> Console.WriteLine(p));
